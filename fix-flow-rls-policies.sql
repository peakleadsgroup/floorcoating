...
-- This uses DROP IF EXISTS to avoid errors if policies already exist

-- Policies for flow_steps (allow anon users)
DROP POLICY IF EXISTS "Allow anon users to read flow steps" ON flow_steps;
CREATE POLICY "Allow anon users to read flow steps"
  ON flow_steps FOR SELECT
  TO anon
  USING (true);

DROP POLICY IF EXISTS "Allow anon users to insert flow steps" ON flow_steps;
CREATE POLICY "Allow anon users to insert flow steps"
  ON flow_steps FOR INSERT
  TO anon
  WITH CHECK (true);

DROP POLICY IF EXISTS "Allow anon users to update flow steps" ON flow_steps;
...
DROP POLICY IF EXISTS "Allow anon users to delete flow steps" ON flow_steps;
...
DROP POLICY IF EXISTS "Allow anon users to read message logs" ON message_logs;
